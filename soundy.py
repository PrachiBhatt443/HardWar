...
class Recorder:
    ...
    def __init__(self):
        ...
        # Load beep sound
        self.beep_sound = wave.open('beep.wav', 'rb')
        self.beep_stream = self.p.open(format=self.p.get_format_from_width(self.beep_sound.getsampwidth()),
                                       channels=self.beep_sound.getnchannels(),
                                       rate=self.beep_sound.getframerate(),
                                       output=True)
    ...
    def update_plot(self, rms_val):
        ...
        # Trigger alert and play beep if RMS value exceeds threshold
    ...
    def play_beep(self):
        # Rewind beep sound to the start
        self.beep_sound.rewind()

        # Read and play beep sound frames
        data = self.beep_sound.readframes(CHUNK)
        while data != b'':
            self.beep_stream.write(data)
            data = self.beep_sound.readframes(CHUNK)
